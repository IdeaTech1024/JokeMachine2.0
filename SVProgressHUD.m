...
@property (nonatomic, strong, readonly) UIImageView *spinnerView2;
...
@synthesize overlayView, hudView, maskType, fadeOutTimer, stringLabel, imageView, /*spinnerView,*/ spinnerView2, visibleKeyboardHeight;
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 50000
...
@synthesize hudNetErrorImage = _uiHudNetErrorImage;
#endif
...
+ (void)showNetErrorWithStatus:(NSString *)string {
    [self showImage:[[self sharedView] hudNetErrorImage] status:string];
}
...
+ (void)showWithOnlyStatus:(NSString *)string {
...
//		self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
        self.spinnerView2.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
...
//		self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, ceil(self.hudView.bounds.size.height/2)+0.5);
        self.spinnerView2.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, ceil(self.hudView.bounds.size.height/2)+0.5);
...
//        [self.spinnerView2 stopLoading];
        self.spinnerView2.hidden = YES;
...
//        [self.spinnerView2 startLoading];
        [self.spinnerView2.layer spinArrowImage];
    }
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return (float)string.length*0.06 + 1.5;
}
...
- (UIImageView *)imageView {
    if (imageView == nil){
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 39, 39)];
...
- (UIImageView *)spinnerView2 {
    if (spinnerView2 == nil) {
        spinnerView2 = [[UIImageView alloc] init];
        spinnerView2.backgroundColor = [UIColor clearColor];
        spinnerView2.image = [UIImage imageNamed:@"SVProgressHUD.bundle/load.png"];
		spinnerView2.bounds = CGRectMake(0, 0, 35, 35);
    }
    
    if(!spinnerView2.superview)
        [self.hudView addSubview:spinnerView2];
...
- (UIImage *)hudNetErrorImage {
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 50000
    if(_uiHudNetErrorImage == nil) {
        _uiHudNetErrorImage = [[[self class] appearance] hudNetErrorImage];
    }
    
    if(_uiHudNetErrorImage != nil) {
        return _uiHudNetErrorImage;
    }
#endif
    
    return [UIImage imageNamed:@"SVProgressHUD.bundle/netError.png"];
}
